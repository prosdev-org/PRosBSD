...
int read_config(ConfigVar *vars, int count) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (strncmp(p, vars[i].name, name_len) != 0)
                continue;

            char *after_name = p + name_len;
            if (*after_name != ' ' && *after_name != '\t' && *after_name != ':' && *after_name != '?' &&
                *after_name != '=') {
                continue;
            }

            char *op_start = after_name;
            while (*op_start && isspace(*op_start))
                op_start++;
...
int write_config(ConfigVar *vars, int count) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (strstr(line, vars[i].name) == line || strstr(line, vars[i].name) == line + 1) {
                char new_line[MAX_LINE_LEN];
                if (vars[i].suffix[0] != '\0') {
                    snprintf(new_line, sizeof(new_line), "%s%s %s", vars[i].prefix, vars[i].value, vars[i].suffix);
                } else {
                    snprintf(new_line, sizeof(new_line), "%s%s", vars[i].prefix, vars[i].value);
                }
                fprintf(temp, "%s\n", new_line);
                replaced = 1;
                changed = 1;
